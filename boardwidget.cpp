...
void BoardWidget::drawShipShape(QPainter &p, int size, Orientation orient, QRect rect, bool isEnemy, bool isDestroyed)
{
    ...
    if (isDestroyed) { mainColor = QColor(40, 40, 40); deckColor = QColor(20, 20, 20); borderColor = QColor(200, 50, 50); }
    p.save();
    if (orient == Orientation::Vertical) {
        p.translate(rect.center()); p.rotate(90);
        int w = rect.height(); int h = rect.width();
        QRect localRect(-w/2, -h/2, w, h); int gap = 4;
        QRect body = localRect.adjusted(gap, gap, -gap, -gap);
        p.setPen(QPen(borderColor, 2)); p.setBrush(mainColor); p.drawRoundedRect(body, 5, 5);
        p.setPen(Qt::NoPen); p.setBrush(deckColor);
        for(int i=0; i<size; ++i) {
            int cx = body.left() + (body.width() / size) * i + (body.width()/size)/2;
            int cy = body.center().y();
            if (i == 1 && size > 2) p.drawRect(cx - 4, cy - 6, 8, 12); else p.drawEllipse(QPoint(cx, cy), 3, 3);
        }
    } else {
        int gap = 4; QRect body = rect.adjusted(gap, gap, -gap, -gap);
        p.setPen(QPen(borderColor, 2)); p.setBrush(mainColor); p.drawRoundedRect(body, 5, 5);
        p.setPen(Qt::NoPen); p.setBrush(deckColor);
        for(int i=0; i<size; ++i) {
            int cx = body.left() + (body.width() / size) * i + (body.width()/size)/2;
...
void BoardWidget::drawExplosion(QPainter &p) {
    ...
    int radius = 5 + f * 2;
    p.setPen(Qt::NoPen); p.setBrush(color2); p.drawRect(pos.x() - radius, pos.y() - radius, radius*2, radius*2);
    int innerR = radius * 0.6; p.setBrush(color1); p.drawRect(pos.x() - innerR, pos.y() - innerR, innerR*2, innerR*2);
    if (f < 15) {
